...
public class lab3 {
	...
	public static String recoverMessage(BigInteger N1, BigInteger N2, BigInteger N3, BigInteger C1, BigInteger C2, BigInteger C3){
		...
		BigInteger ProductN = N1.multiply(N2).multiply(N3);

		BigInteger Num_N1 = ProductN.divide(N1);
		BigInteger Num_N2 = ProductN.divide(N2);
		BigInteger Num_N3 = ProductN.divide(N3);

		BigInteger Inverse_Num_N1 = Num_N1.modInverse(N1);
		BigInteger Inverse_Num_N2 = Num_N2.modInverse(N2);	
		BigInteger Inverse_Num_N3 = Num_N3.modInverse(N3);

		BigInteger X1 = (C1.multiply(Num_N1)).multiply(Inverse_Num_N1);
		BigInteger X2 = (C2.multiply(Num_N2)).multiply(Inverse_Num_N2);
		BigInteger X3 = (C3.multiply(Num_N3)).multiply(Inverse_Num_N3);

		BigInteger X = (X1.add(X2).add(X3)).mod(ProductN);
		
		
		
		BigInteger messageInt = findCubeRoot(X);
